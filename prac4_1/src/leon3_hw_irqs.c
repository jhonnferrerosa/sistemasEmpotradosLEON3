...
uint8_t leon3_clear_irq (uint8_t irq_level) {
	...
	if((irq_level >0) && (irq_level <16)){
		...
		*LEON3_ICLEAR = *LEON3_ICLEAR | bit;	//Sets that bit to 1
		//*LEON3_ICLEAR = *LEON3_ICLEAR & ~(bit);  //duda?? este es el que hay que poner?.
	}else
		error=1;
	return error;
}
...
uint8_t leon3_unmask_irq (uint8_t irq_level){
	...
	if((irq_level >0) && (irq_level <16)){
		...
		// *LEON3_IMASK = 0; profe: lo borras todo.
		uint32_t bit = (1 << irq_level);
		*LEON3_IMASK |= bit;
	}else
		error=1;
...
uint8_t leon3_mask_irq (uint8_t irq_level) {
	...
	if(irq_level >0 && irq_level <16){
		...
		//*LEON3_IMASK  = 0xFFFE;  // profe: lo cambias todo.
		uint32_t bit = (1 << irq_level);
		bit =~ bit;   //profe:  MEJOR bit = ~bit;
		*LEON3_IMASK &= bit;
	}else
		error=1;
	return error;

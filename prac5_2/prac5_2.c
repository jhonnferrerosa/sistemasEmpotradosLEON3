...
rtems_task Init(rtems_task_argument argument){
  ...
  status = rtems_task_start( Task_id[ 2 ], TPathTracking, 1 );
  status = rtems_task_start( Task_id[ 3 ], TSensorFusion, 1 );
...
rtems_task TAvoidObstacles(rtems_task_argument unused){
  rtems_status_code status;
  rtems_interval ticksDuracionTarea = 10;
  rtems_interval tiksActuales = get_ticks_since_boot(); //---anadido planificador ciclico sin derivas.
  for ( ; ; ) {
		puts("T1 Do Avoid Obstacles");
		printf(" - rtems_ticks_since_boot - %i\n\n", get_ticks_since_boot());
		tiksActuales = tiksActuales + ticksDuracionTarea;  //---anadido planificador ciclico sin derivas.
		status = rtems_task_wake_after (ticksDuracionTarea); //---anadido planificador ciclico sin derivas.
		//status = task_delay_until (ticksDuracionTarea);
  }
}
rtems_task TPathTracking(rtems_task_argument unused){
	rtems_status_code status;
	rtems_interval ticksDuracionTarea = 15;
	rtems_interval tiksActuales = get_ticks_since_boot();
	for ( ; ; ) {
		puts("T2 Do PathTracking");
		printf(" - rtems_ticks_since_boot - %i\n\n", get_ticks_since_boot());
		tiksActuales = tiksActuales + ticksDuracionTarea;
		status = rtems_task_wake_after (ticksDuracionTarea);
		//status = task_delay_until (ticksDuracionTarea);
  }
}
rtems_task TSensorFusion(rtems_task_argument unused){
	rtems_status_code status;
	rtems_interval ticksDuracionTarea = 30;
	rtems_interval tiksActuales =  get_ticks_since_boot();
	for ( ; ; ) {
